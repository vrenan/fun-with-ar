...
public class AugmentedImageNode extends AnchorNode {
  ...
  private static CompletableFuture<ViewRenderable> viewRenderableFuture;

  public AugmentedImageNode(Context context) {
    // Upon construction, start loading the models for the corners of the frame.
    if (viewRenderableFuture == null) {
      viewRenderableFuture =
              ViewRenderable.builder().setView(context, R.layout.solar_controls)
  ...
  public void setImage(AugmentedImage image) {
    ...
    final float maze_edge_size = 0.5f;
    final float max_image_edge = Math.max(image.getExtentX(), image.getExtentZ());
    maze_scale = max_image_edge / maze_edge_size;

    cornerNode.setLocalScale(new Vector3(maze_scale*0.8f, maze_scale*0.8f, maze_scale*0.8f));
    cornerNode.setLocalRotation(new Quaternion( new Vector3(1f, 0f, 0f), -90f));
    cornerNode.setParent(this);
    cornerNode.setRenderable(viewRenderableFuture.getNow(null));
